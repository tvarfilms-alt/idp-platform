...
def fetch_ticker_ratios(ticker: str) -> dict | None:
    ...
    try:
        ...
        # Debug: log actual response structure for first few calls
        if not hasattr(fetch_ticker_ratios, '_debug_count'):
            fetch_ticker_ratios._debug_count = 0
        if fetch_ticker_ratios._debug_count < 3:
            fetch_ticker_ratios._debug_count += 1
            top_keys = list(data.keys()) if isinstance(data, dict) else f"type={type(data).__name__}"
            inc = data.get("included")
            inc_info = "None"
            if isinstance(inc, dict):
                inc_info = f"dict keys={list(inc.keys())}"
            elif isinstance(inc, list):
                inc_info = f"list len={len(inc)}"
                if inc:
                    inc_info += f" first_keys={list(inc[0].keys()) if isinstance(inc[0], dict) else type(inc[0]).__name__}"
            log.info(f"DEBUG {ticker}: top_keys={top_keys}, included={inc_info}")

        ratios_list = data.get("included", {}).get("ratios", [])
        if not ratios_list:
