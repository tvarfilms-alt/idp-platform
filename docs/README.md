...
- **3 кнопки** 🔴 🟡 🟢 — оценка дня.
- **Синхронизация** — Firebase Firestore (real-time между телефонами).
...
4. **Build → Authentication → Get started → Sign-in method → Anonymous →
   Enable.**
5. **Build → Firestore Database → Create database** (можно «production mode»,
   регион любой).
6. Вкладка **Firestore → Rules**: вставь содержимое
   [`docs/firestore.rules`](./firestore.rules) и нажми **Publish**.
...
repository secret** и добавь 4 секрета:
