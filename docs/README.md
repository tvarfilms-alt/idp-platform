...
repository secret** и добавь 2 секрета (остальное захардкожено в `push/send.js`,
т.к. это не секретные значения):

| Имя | Значение |
|-----|----------|
| `VAPID_PRIVATE_KEY` | приватный VAPID-ключ (его дал ассистент; можно перегенерировать) |
